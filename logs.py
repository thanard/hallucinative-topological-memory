...
def log_images(o, o_pred, o_neg, c, test_context, model, c_model, n_contexts, n_samples_per_c, savepath, epoch):
    ...
    # save image
    save_image(torch.Tensor(sorted_mask_o), os.path.join(savepath, 'sample_data_' + str(epoch) + '.png'), nrow=n_contexts)
...
def score_images(sample_o, o_pred, c, c_model, n_samples_per_c, n_contexts):
    ...
    sorted_mask_o = np.concatenate([o_pred.detach().cpu().numpy(), sorted_mask_o.reshape(-1, *mask_o.shape[2:])], axis=0)
